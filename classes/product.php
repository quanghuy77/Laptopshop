...
<?php
	...
	class product
	{
		...
		public function insert_product($data,$files){
			...
			$file_ext = strtolower(end($div)); /* chuyển các ký tự thành chữ thường */
			$unique_image = substr(md5(time()), 0, 10).'.'.$file_ext;	/* băm tên random số từ 0-10 */
			$uploaded_image = "uploads/".$unique_image;	/* upload lên file uploads */
		...
		public function update_product($data,$files,$id){
			...
			$div = explode('.', $file_name); /* phân tách tên với đuôi */
			$file_ext = strtolower(end($div));
			// $file_current = strtolower(current($div));
			$unique_image = substr(md5(time()), 0, 10).'.'.$file_ext;
			$uploaded_image = "uploads/".$unique_image;
		...
		public function getproduct_feathered(){
			$query = "SELECT * FROM tbl_product where type = '0' order by RAND() LIMIT 10 ";
			$result = $this->db->select($query);
			return $result;
		} 
		...
		public function del_compare($proid, $customer_id){
			$sId = session_id();
			$query = "DELETE FROM tbl_compare WHERE customer_id = '$customer_id' and productId = '$proid'";
			$result = $this->db->delete($query);
			return $result;
		}
	/*  thêm vào yêu thích */
